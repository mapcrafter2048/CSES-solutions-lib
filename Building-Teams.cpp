...
bool dfs(int node, vector<int> &col, vector<vector<int>> &adj, int parent) {

    if (parent == -1)
        col[node] = 1;
    else
        col[node] = col[parent] ^ 3;

    for (auto child : adj[node]) {
        if (child == parent)
            continue;      // checking if the child is parent or not...
        if (!col[child]) { // if child is not coloured...
            if (!dfs(child, col, adj, node))
                return false;
        }
        if (col[child] ==
            col[node]) // child's and current node's colour is same...
            return false;
    }
